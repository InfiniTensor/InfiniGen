...
static const bool dlog_switch = getBoolEnvironment("TG_DLOG", false);
static const int32_t dlog_level =
    getLevelEnvironment("TG_DLOG_LEVEL", 0) < 0
        ? 0
        : (getLevelEnvironment("TG_DLOG_LEVEL", 0) > dlog_total_level
               ? dlog_total_level
               : getLevelEnvironment("TG_DLOG_LEVEL", 0));
...
Log::Log(std::string file, int32_t line, int32_t severity, int32_t module,
    ...
    log_module = module < 0
                     ? 0
                     : (module > dlog_total_level ? dlog_total_level : module);
    module_name = name;
}
